...
function trackConsecutive(id){
  if(player.state.consecutiveAction===id){
    player.state.consecutiveCount=(player.state.consecutiveCount||0)+1;
  } else {
    player.state.consecutiveAction=id;
    player.state.consecutiveCount=1;
  }
}
...
function actionTitle(id){
  return {
    study:'공부했다',play:'놀았다',club:'동아리 방에 갔다',rest:'쉬었다',
    partTime:'알바를 했다',team:'팀플을 붙들었다',toeic:'어학 준비를 했다',
    intern:'인턴 지원서를 열었다',contest:'공모전에 집중했다',
    cert:'자격증 공부를 했다',date:'데이트를 했다',job:'취업 준비를 했다',
    interview:'면접을 상상했다',grad:'대학원을 고민했다',military:'입대를 고민했다',
    cram:'벼락치기를 했다',borrow:'친구 필기를 빌렸다',library:'도서관에 박혔다',
    guess:'시험을 감으로 봤다',sleep:'시험 날 잤다',
    askFriend:'인맥을 동원했다',useCareer:'경험을 활용했다',
    grind:'체력으로 밀어붙였다',training:'훈련에 집중했다'
  }[id]||'이번 달을 보냈다';
}
...
function chooseMain(choice){
  ...
  if(consEv&&chance(0.7)){
    player.state.consecutiveCount=0;
    return popupEvent(consEv,'general');
  }

  if(choice.id==='rest') maybeRumorPopup();
  if(choice.id==='partTime') player.state.partTimeJob=true;
  if(choice.id==='rest'||choice.id==='sleep'){
    player.state.oversleptBoost=true;
    if(typeof maybeHousingEvent==='function'&&chance(0.28)){ if(maybeHousingEvent()) return; }
...
function popupEvent(eventObj,variant){
  ...
  if(!rawChoices||!rawChoices.length){
    rawChoices=getDefaultChoicesForCategory(eventObj.category);
  }
...
applyEffect(effect);
  ...
  openModal(`✨ 이벤트: ${eventObj.title}`,eventObj.description,choices,variant);
}
  ...
  if(ev){
    ...
    if(player.state.oversleptBoost){
      ...
      return popupEvent({
        ...
        choices:[
          {text:'그냥 오늘은 버린다',effect:{stress:-4,gradePoint:-8,health:2}},
          {text:'늦었지만 간다',effect:{stress:5,gradePoint:2,health:-2}},
          {text:'공강이라 합리화한다',effect:{stress:-1,gradePoint:-5,relationship:-1}},
          {text:'친구에게 출석 부탁한다',effect:{relationship:3,stress:2,gradePoint:-3}}
        ]
      },'general');
    }
    // 찌라시 연계 이벤트
    if(player.state.lastRumorId==='r6'&&chance(0.6)){
      ...
      return popupEvent({
        title:'📄 족보를 직접 받았다',
        description:'소문이 사실이었다. 누군가 카톡으로 파일을 보내줬다.',
        choices:[{text:'믿고 공부한다',effect:{gradePoint:6,stress:-3}},{text:'반신반의하며 본다',effect:{gradePoint:2,stress:1}},{text:'쓰레기통에 버린다',effect:{gradePoint:-1}}]
      },'general');
    }
    if(player.state.lastRumorId==='r4'&&chance(0.5)){
      ...
      return popupEvent({
        title:'💘 소문의 당사자를 마주쳤다',
        description:'소문이 사실이었다. 생각보다 잘 어울렸다.',
        choices:[{text:'축하해준다',effect:{relationship:4,love:2}},{text:'괜히 묘한 기분',effect:{love:3,stress:2}},{text:'모른 척한다',effect:{stress:-1}}]
      },'general');
    }
    if(player.state.lastRumorId==='r10'&&chance(0.55)){
      ...
      return popupEvent({
        title:'🎤 콘서트 티켓팅에 성공했다',
        description:'기적이 일어났다. 손이 떨렸다.',
        choices:[{text:'간다!!',effect:{otaku:8,money:-8,stress:-8}},{text:'공부 때문에 판다',effect:{money:10,stress:2}},{text:'친구와 같이 간다',effect:{otaku:6,relationship:5,money:-8}}]
      },'general');
    }
    if(typeof maybeHousingEvent==='function'&&chance(0.22)){
...
function mandatoryMajorEvent(){
  ...
  const domainEvents={
    engineering:{
      2:{title:'⚙️ 설계 과제 지옥 시작',desc:`${player.profile.majorLabel} 2학년, 드디어 실전 설계가 시작됐다.\n밤을 새워도 뭔가 항상 부족하다. CAD와 친해지는 시간.`},
      3:{title:'🔧 캡스톤 디자인 킥오프',desc:`졸업작품이 현실이 됐다. 팀을 꾸리고, 주제를 정하고.\n이게 끝나면 진짜 어른이 되는 느낌이다.`}
    },
    art:{
      2:{title:'🎨 포트폴리오 첫 리뷰',desc:`교수님 앞에서 작품 리뷰. 칭찬인지 비판인지 모를 피드백이 돌아왔다.\n예술가는 비판에 단단해져야 한다고 한다.`},
      3:{title:'🖼️ 전시 준비가 시작됐다',desc:`아이디어는 있는데 재료비와 시간이 문제다.\n예술은 낭만이고 현실은 예산이다.`}
    },
    medical:{
      2:{title:'🔬 첫 실습이 시작됐다',desc:`이론으로만 배운 걸 직접 마주하게 됐다.\n생각보다 담담했고, 또 생각보다 무거웠다.`},
      3:{title:'🏥 현장 실습 배정',desc:`드디어 현장이다. 아는 것과 할 수 있는 것 사이의 거리를 몸으로 배우는 시간.`}
    },
    business:{
      2:{title:'📊 케이스 스터디 발표',desc:`실제 기업 케이스를 분석해서 발표한다.\n정답이 없는 게 더 어렵다. 자신 있게 말하는 연습이 필요하다.`},
      3:{title:'🚀 창업 경진대회 권유',desc:`교수님이 팀을 꾸려서 나가보라 했다.\n기회인지 짐인지 아직 모르겠다. 둘 다일 수도 있다.`}
    },
    humanities:{
      2:{title:'📚 논문 읽기 과제 폭탄',desc:`A4 30장짜리 영문 논문 5편이 숙제다.\n학문의 세계는 넓고 내 시간은 짧다.`},
      3:{title:'🎓 학회 발표 제안',desc:`교수님이 학회에서 발표해보겠냐고 했다.\n영광인데 긴장된다. 준비가 돼 있을까?`}
    },
    sports:{
      2:{title:'🏃 전국 대회 출전 기회',desc:`${player.profile.majorLabel} 2학년, 처음으로 전국 대회 출전 기회가 왔다.\n훈련한 만큼 나온다는 걸 이번에 확인하게 된다.`},
      3:{title:'🏆 스포츠 에이전트와 면담',desc:`드래프트 전 에이전트 미팅이 잡혔다.\n진로가 구체적으로 보이기 시작했다.`}
    },
    general:{
      2:{title:`${player.profile.majorLabel} 2학년 이벤트`,desc:`${player.profile.majorLabel}다운 사건이 찾아왔다.\n전공이 이제 진짜 자기 얼굴을 드러내기 시작했다.`},
      3:{title:`${player.profile.majorLabel} 3학년 이벤트`,desc:`이제는 전공이 곧 진로처럼 느껴지는 순간이다.\n잘하면 기회가 되고, 못하면 그냥 피곤한 추억이 된다.`}
    }
  };
  ...
  if(grade===2&&!player.state.majorEvent2Done){
    ...
    return {title:ev.title,description:ev.desc,choices:[
      {text:'정면으로 부딪힌다',effect:{career:7,gradePoint:2,stress:5}},
      {text:'조심스럽게 참여한다',effect:{career:4,stress:2}},
      {text:'무리하지 않는다',effect:{stress:-3,career:-1}}
    ],variant:'general'};
  }
  if(grade===3&&!player.state.majorEvent3Done){
    player.state.majorEvent3Done=true;
    return {title:ev.title,description:ev.desc,choices:[
      {text:'성과를 만든다',effect:{career:8,stress:5,money:-2}},
      {text:'사람과 같이 해본다',effect:{career:5,relationship:5,stress:3}},
      {text:'일단 배운다',effect:{gradePoint:3,career:3,stress:2}}
    ],variant:'general'};
  }
  return null;
}
...
function continueChainIfNeeded(){
  ...
  if(chain.type==='military'){
    const idx=Math.min(chain.step,MILITARY_EVENTS.length-1);
    chain.step+=1;
    if(chain.step<5){popupEvent(MILITARY_EVENTS[idx],'military');return true;}
    player.state.activeChain=null;return false;
  }
  if(chain.type==='rivalLove'){
    const steps=(typeof getRivalLoveChainEvents==='function')?getRivalLoveChainEvents():[];
    if(chain.step<steps.length){popupEvent(steps[chain.step],'love');chain.step+=1;return true;}
    player.state.activeChain=null;player.state.rivalRomance=true;return false;
  }
  if(chain.type==='story'){
    const steps=(typeof getStoryRouteEvents==='function')?getStoryRouteEvents(chain.routeKey):[];
    if(chain.step<steps.length){popupEvent(steps[chain.step],'special');chain.step+=1;return true;}
    player.state.activeChain=null;return false;
  }
  if(chain.type==='job'){
    const paths={
      parttime:[
        {title:'알바 매니저 제안',description:'사장님이 오래 해볼 생각 없냐고 진지하게 물었다.',choices:[{text:'계속 해본다',effect:{career:4,money:8},special:'start_parttime'},{text:'일단 넘긴다',effect:{money:2}},{text:'그만둔다',effect:{stress:-2}}]},
        {title:'정직원 제안',description:'점장이 정직원 얘기를 꺼냈다. 이상하게 현실감이 확 든다.',choices:[{text:'받아들인다',effect:{career:12,money:15,stress:8}},{text:'조건을 더 본다',effect:{career:6,money:4}},{text:'공부를 택한다',effect:{gradePoint:3,career:-2}}]}
      ],
      randomhire:[
        {title:'아무 데나 넣었는데 연락이 왔다',description:'진짜 될 줄은 몰랐는데 갑자기 면접 연락이 와버렸다.',choices:[{text:'일단 가본다',effect:{career:8,stress:5}},{text:'무섭지만 준비한다',effect:{career:10,stress:7}},{text:'그냥 포기한다',effect:{stress:-1,career:-3}}]},
        {title:'갑자기 합격 통보',description:'정말 예상 못 한 곳에서 합격 문자가 왔다. 인생은 가끔 서사를 생략한다.',choices:[{text:'입사한다',effect:{career:14,money:12}},{text:'조금 더 고민한다',effect:{stress:4}},{text:'더 좋은 곳을 본다',effect:{career:4}}]}
      ]
    };
