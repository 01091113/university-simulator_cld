...
function getStressBadge(stress){if(stress<30)return{text:'😌 평온',cls:''};if(stress<55)return{text:'😅 약간 긴장',cls:''};if(stress<70)return{text:'😰 스트레스 주의',cls:'warning'};if(stress<88)return{text:'🤯 스트레스 과다',cls:'warning'};return{text:'💀 사망 직전',cls:'danger'}}
...
function renderMajorInfo(){
  ...
  document.getElementById('major-info-box').innerHTML=`<strong>${data.name}</strong><br><span style="color:var(--sub);font-size:14px">${data.desc}</span><br><br><strong>보정치:</strong> ${bonus}`;
}
...
function renderStatus(){
  ...
  const gradeColor=g>=3.5?'#4ade80':g>=2.5?'#facc15':'#f87171';
  document.getElementById('status-list').innerHTML=`
    <div class="status-grid">
      ...
      <div class="status-card"><div class="status-head"><span>📚 학점</span><span style="color:${gradeColor}">${g} / 4.5</span></div></div>
...
function openShop(){
  ...
  const items=[
    ...
    {id:'plush',name:'🧸 작은 인형',price:12,desc:'스트레스 -6 (심리적 위로)'},
    {id:'coffee',name:'☕ 프리미엄 커피',price:6,desc:'스트레스 -3, 취업역량 +2 (카공 감성)'},
    {id:'ring',name:'💍 반지',price:25,desc:'연애 루트 고백 성공에 필요'},
    {id:'lotto',name:'🎰 로또',price:3,desc:'낮은 확률로 돈 +150. 인생역전의 꿈.'},
    {id:'vitaminPack',name:'💊 비타민 세트',price:10,desc:'체력 +5, 스트레스 -3, 건강 루틴 시작'},
    {id:'study_kit',name:'📦 수험생 패키지',price:15,desc:'학점 +3, 스트레스 -5 (족보 포함)'}
  ];
  document.getElementById('shop-list').innerHTML=items.map(item=>`
    <div class="shop-item">
      <div class="row">
        <strong>${item.name}</strong>
        <button class="secondary-btn small" onclick="buyItem('${item.id}')">구매 (💸${item.price})</button>
      </div>
      <p>${item.desc}</p>
    </div>`).join('');
...
function renderRumors(){
  document.getElementById('rumor-list').innerHTML=(player.state.rumors||[]).map(r=>`<div class="rumor-item"><strong>📱 ${r.title}</strong><br><span style="font-size:14px;color:var(--sub)">${r.body}</span></div>`).join('')||'<div class="rumor-item">아직 쌓인 찌라시가 없다.</div>';
}
